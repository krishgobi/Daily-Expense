...
export const ExpenseEditForm: React.FC<ExpenseEditFormProps> = ({ expenseId, onSuccess, onCancel }) => {
  ...
  useEffect(() => {
    expenseService.getExpense(expenseId).then((e: Expense) => {
      ...
      setPaymentMethod((e as any).payment_method || 'GPay')
      setExistingMedia((e as any).media || [])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true)
    try {
      const updates: any = { purpose, amount: numAmount, date, description: description || undefined, location: location || undefined }
      if (expenseType === 'DIGITAL') updates.payment_method = paymentMethod
    ...
    <div className="space-y-5">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <FormField label="Purpose" required>
          <Input type="text" value={purpose} onChange={(e) => setPurpose(e.target.value)} placeholder="e.g. Lunch, Gas" required />
        </FormField>
        ...
        <FormField label="Location">
          <Input type="text" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="Where did you spend?" />
        </FormField>
        ...
        {/* Upload toggle for digital */}
        {expenseType === 'DIGITAL' && showUpload && (
        ...
        <div className="flex gap-3 pt-1">
          ...
          {expenseType === 'DIGITAL' && (
            <Button type="button" variant="ghost" onClick={() => setShowUpload((v) => !v)}>
